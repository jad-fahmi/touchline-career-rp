...
FIFA never records who a played match was against; the only trace is a news article, and the game keeps roughly twenty of those before discarding the oldest. Touchline recovers the opponent in three steps, in order of cost: the article itself, a fixture captured on an earlier scan, and finally the running game, whose memory still holds the club, the opponent, the date, and the score. The live read happens only when the first two fail, is strictly read-only, and is skipped when FIFA is closed, so a match imports on its own instead of waiting for you to name it.

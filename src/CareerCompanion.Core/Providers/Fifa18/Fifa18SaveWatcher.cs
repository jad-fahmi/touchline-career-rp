...
public sealed class Fifa18SaveWatcher : IDisposable
{
    ...
    public Fifa18SaveWatcher(string directory)
    {
        ...
        _watcher.Changed+=Changed;_watcher.Created+=Changed;_watcher.Renamed+=Changed;_watcher.Error+=(_,e)=>RaiseError(e);_watcher.EnableRaisingEvents=true;
    }
    ...
    private void RaiseError(ErrorEventArgs error){lock(_gate){if(_disposed)return;WatcherError?.Invoke(this,error);}}

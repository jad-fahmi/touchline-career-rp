...
public sealed class Database(string path)
{
    ...
    public void Migrate()
    {
        ...
        cmd.CommandText = """
            ...
            CREATE TABLE IF NOT EXISTS matches(id INTEGER PRIMARY KEY, career_id INTEGER NOT NULL REFERENCES careers(id) ON DELETE CASCADE,
              ...
              team_context TEXT NOT NULL DEFAULT 'Club', representing_team TEXT NOT NULL DEFAULT '');
            ...
            CREATE TABLE IF NOT EXISTS fixtures(id INTEGER PRIMARY KEY, career_id INTEGER NOT NULL REFERENCES careers(id) ON DELETE CASCADE,
              ...
              representing_team TEXT NOT NULL DEFAULT '', UNIQUE(career_id,provider,event_key));
    ...
    {
        using var db = Open(); using var cmd = db.CreateCommand();
        cmd.CommandText = "INSERT INTO matches(career_id,date,competition,opponent,is_home,team_score,opponent_score,started,minutes,goals,assists,rating,yellow_card,red_card,penalty_scored,penalty_missed,notes,next_opponent,is_derby,is_major,result,created_at,started_known,team_context,representing_team) " +
          "VALUES($career,$date,$comp,$opp,$home,$ts,$os,$started,$min,$goals,$assists,$rating,$yellow,$red,$ps,$pm,$notes,$next,$derby,$major,$result,$now,$startedKnown,$context,$team); SELECT last_insert_rowid();";
        var values = new Dictionary<string,object?> { ["$career"]=careerId,["$date"]=m.Date,["$comp"]=m.Competition,["$opp"]=m.Opponent,["$home"]=m.IsHome,["$ts"]=m.TeamScore,["$os"]=m.OpponentScore,["$started"]=m.Started,["$min"]=m.Minutes,["$goals"]=m.Goals,["$assists"]=m.Assists,["$rating"]=m.Rating,["$yellow"]=m.YellowCard,["$red"]=m.RedCard,["$ps"]=m.PenaltyScored,["$pm"]=m.PenaltyMissed,["$notes"]=m.Notes,["$next"]=m.NextOpponent,["$derby"]=m.IsDerby,["$major"]=m.IsMajorFixture,["$result"]=m.TeamScore>m.OpponentScore?"W":m.TeamScore<m.OpponentScore?"L":"D",["$now"]=DateTime.UtcNow.ToString("O"),["$startedKnown"]=m.StartedKnown,["$context"]=m.TeamContext,["$team"]=m.RepresentingTeam };
    ...
    public void UpdateMatch(long careerId,long matchId,MatchInput m)
    {
        using var db=Open();using var cmd=db.CreateCommand();cmd.CommandText="""
            UPDATE matches SET date=$date,competition=$comp,opponent=$opp,is_home=$home,team_score=$ts,opponent_score=$os,
              ...
              result=$result,started_known=$startedKnown,team_context=$context,representing_team=$team WHERE career_id=$career AND id=$match
            """;
        var values=new Dictionary<string,object?>{{"$career",careerId},{"$match",matchId},{"$date",m.Date},{"$comp",m.Competition},{"$opp",m.Opponent},{"$home",m.IsHome},{"$ts",m.TeamScore},{"$os",m.OpponentScore},{"$started",m.Started},{"$min",m.Minutes},{"$goals",m.Goals},{"$assists",m.Assists},{"$rating",m.Rating},{"$yellow",m.YellowCard},{"$red",m.RedCard},{"$ps",m.PenaltyScored},{"$pm",m.PenaltyMissed},{"$notes",m.Notes},{"$next",m.NextOpponent},{"$derby",m.IsDerby},{"$major",m.IsMajorFixture},{"$result",m.TeamScore>m.OpponentScore?"W":m.TeamScore<m.OpponentScore?"L":"D"},{"$startedKnown",m.StartedKnown},{"$context",m.TeamContext},{"$team",m.RepresentingTeam}};
        ...
        bool isHome,int confidence,string evidence,string sourceFingerprint,string teamContext="Club",string representingTeam="")
    {
        ...
        using var cmd=db.CreateCommand();cmd.Transaction=tx;cmd.CommandText="INSERT INTO fixtures(career_id,provider,event_key,date,competition,opponent,is_home,status,confidence,evidence,source_fingerprint,updated_at,team_context,representing_team) VALUES($c,$p,$event,$date,$competition,$opponent,$home,'Upcoming',$confidence,$evidence,$fingerprint,$now,$context,$team) ON CONFLICT(career_id,provider,event_key) DO UPDATE SET date=excluded.date,competition=excluded.competition,opponent=excluded.opponent,is_home=excluded.is_home,status='Upcoming',confidence=excluded.confidence,evidence=excluded.evidence,source_fingerprint=excluded.source_fingerprint,updated_at=excluded.updated_at,team_context=excluded.team_context,representing_team=excluded.representing_team";
        cmd.Parameters.AddWithValue("$c",careerId);cmd.Parameters.AddWithValue("$p",provider);cmd.Parameters.AddWithValue("$event",eventKey);cmd.Parameters.AddWithValue("$date",date);cmd.Parameters.AddWithValue("$competition",competition);cmd.Parameters.AddWithValue("$opponent",opponent);cmd.Parameters.AddWithValue("$home",isHome);cmd.Parameters.AddWithValue("$confidence",confidence);cmd.Parameters.AddWithValue("$evidence",evidence);cmd.Parameters.AddWithValue("$fingerprint",sourceFingerprint);cmd.Parameters.AddWithValue("$now",DateTime.UtcNow.ToString("O"));cmd.Parameters.AddWithValue("$context",teamContext);cmd.Parameters.AddWithValue("$team",representingTeam);cmd.ExecuteNonQuery();tx.Commit();
    }

    public IReadOnlyList<CareerFixture> GetFixtures(long careerId,int limit=50)
    {
        using var db=Open();using var cmd=db.CreateCommand();cmd.CommandText="SELECT id,career_id,provider,event_key,date,competition,opponent,is_home,status,confidence,evidence,updated_at,team_context,representing_team FROM fixtures WHERE career_id=$c ORDER BY CASE status WHEN 'Upcoming' THEN 0 ELSE 1 END,date DESC,id DESC LIMIT $limit";cmd.Parameters.AddWithValue("$c",careerId);cmd.Parameters.AddWithValue("$limit",limit);using var r=cmd.ExecuteReader();var result=new List<CareerFixture>();while(r.Read())result.Add(new(r.GetInt64(0),r.GetInt64(1),r.GetString(2),r.GetString(3),r.GetString(4),r.GetString(5),r.GetString(6),r.GetBoolean(7),r.GetString(8),r.GetInt32(9),r.GetString(10),DateTime.Parse(r.GetString(11)),r.GetString(12),r.GetString(13)));return result;
    }

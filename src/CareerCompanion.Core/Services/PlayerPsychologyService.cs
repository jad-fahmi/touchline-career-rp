...
public sealed class PlayerPsychologyService(Database db)
{
    ...
    public static PlayerState CalculateAfterMatch(PlayerState current,CareerMatch match,IReadOnlyList<CareerEvent> events,out int severity)
    {
        ...
        fatigue+=Math.Clamp(match.Input.Minutes/12,1,10);if(match.Result=="W"){confidence+=6;pressure-=5;isolation-=3;resilience+=1;}else if(match.Result=="D"){confidence+=1;pressure+=1;}else if(match.Result=="L"){severity+=6;confidence-=6;pressure+=7;isolation+=3;}

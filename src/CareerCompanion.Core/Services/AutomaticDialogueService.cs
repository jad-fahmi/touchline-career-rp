        ...
        bool premiumRouting,int limit=4,CancellationToken ct=default)
    {
        ...
        foreach(var job in db.GetPendingGenerationJobs(careerId,"automatic_reaction_llm",limit))
        {
            try
            {
                ...
                var scene=character.Type==CharacterType.Manager?SceneType.ManagerOffice:SceneType.PostMatch;var relationship=db.GetRelationship(characterId);
                ...
                var instruction=$"Initiate one natural private message to {career.PlayerName} in direct reaction to this verified event: {evt.Summary} Keep it specific to what is known, stay in character, and do not announce that you are a simulation.";
                var model=premiumRouting&&evt.Importance>=75?premiumModel:defaultModel;
                var request=new PromptBuilder().Character(character,career,relationship,scene,memories,[evt],history,instruction,model,db.GetCharacterState(characterId)) with{MaxOutputTokens=180,Creativity=.75};

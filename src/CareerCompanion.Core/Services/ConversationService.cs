...
public sealed class ConversationService(Database db,ILlmProvider llm)
{
    public async Task<GenerationResult> SendAsync(long careerId,long characterId,SceneType scene,string message,string model,CancellationToken ct=default)
    {
        ...
        GenerationResult result;try{result=await llm.GenerateAsync(new PromptBuilder().Character(character,career,rel,scene,memories,events,history,message,model,db.GetCharacterState(characterId),db.GetPlayerState(careerId)),ct);}catch{db.Log("llm_error",$"Conversation generation failed for character {characterId}");throw;}

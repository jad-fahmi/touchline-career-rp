...
public sealed class ConversationService(Database db,ILlmProvider llm)
{
    public async Task<GenerationResult> SendAsync(long careerId,long characterId,SceneType scene,string message,string model,CancellationToken ct=default)
    {
        ...
        var characterState=db.GetCharacterState(characterId);var playerState=db.GetPlayerState(careerId);GenerationResult result;var usedAi=false;
        ...
        else
        {
            try
            {
                result=await llm.GenerateAsync(new PromptBuilder().Character(character,career,rel,scene,memories,events,history,message,model,characterState,playerState),ct);
                if(!DialogueResponseGuard.IsUsable(result.Text))
                {
                    db.Log("llm_error",$"Conversation response for character {characterId} looked like a prompt echo or malformed dialogue.");
                    result=OfflineReply(character,career,rel,characterState,playerState,message,scene,"AI returned a prompt echo or malformed dialogue");
                }
                else usedAi=true;
            }
            catch(LlmUnavailableException e){db.Log("llm_error",$"Conversation fallback for character {characterId}: {e.Message}");result=OfflineReply(character,career,rel,characterState,playerState,message,scene,e.Message);}
            catch(LlmRateLimitException e){db.Log("llm_error",$"Conversation fallback for character {characterId}: {e.Message}");result=OfflineReply(character,career,rel,characterState,playerState,message,scene,e.Message);}
        }
        ...
        if(usedAi){db.AddUsage(llm.Name,model,result.InputTokens,result.OutputTokens);db.Log("llm",$"Conversation generated; model={model}; input={result.InputTokens}; output={result.OutputTokens}");}
        else db.Log("offline",$"Conversation answered by offline library; character={characterId}; reason={result.Raw["offline-library:".Length..]}");
        return result;
    }
